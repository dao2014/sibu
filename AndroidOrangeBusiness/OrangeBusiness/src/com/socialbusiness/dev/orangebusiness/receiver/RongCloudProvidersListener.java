...
import io.rong.imkit.demo.LocationActivity;
 ...
 */
public final class RongCloudProvidersListener implements RongIM.OnReceiveMessageListener, RongIM.OnSendMessageListener, RongIM.GetUserInfoProvider, RongIM.GetFriendsProvider, RongIM.GetGroupInfoProvider, RongIM.ConversationBehaviorListener, RongIM.ConnectionStatusListener, RongIM.LocationProvider {

    private static final String TAG = RongCloudProvidersListener.class.getSimpleName();

    private static RongCloudProvidersListener mRongCloudInstance;
    ...
    public static void init(Context context) {
        ...
        if (mRongCloudInstance == null) {
            ...
            synchronized (RongCloudProvidersListener.class) {

                if (mRongCloudInstance == null) {
                    mRongCloudInstance = new RongCloudProvidersListener(context);
                }
            }
        }
    }
     ...
     * @param context 上下文。
     */
    private RongCloudProvidersListener(Context context) {
        mContext = context;
        initDefaultListener();
    }
     ...
     * @return RongCloud。
     */
    public static RongCloudProvidersListener getInstance() {
    ...
    public void onReceived(RongIMClient.Message message, int left) {
        ...
        in.setAction(MainActivity.NotifyRefreshReceiver.BROADCAST_NOTIFY_CONVERSATION_CHANGED);
        in.putExtra(UNREAD_COUNT_KEY, RongIM.getInstance().getTotalUnreadCount());
        Log.e("==", "==" + message.getSenderUserId() + "==" + message.getTargetId() + "==" + message.getMessageDirection());
        if (message.getConversationType() == RongIMClient.ConversationType.GROUP)
            in.putExtra(MainActivity.NotifyRefreshReceiver.ISGROUP, DemoContext.getInstance().getGroupMap().get(message.getTargetId()) == null);

        if (message.getConversationType() == RongIMClient.ConversationType.PRIVATE)
            in.putExtra(MainActivity.NotifyRefreshReceiver.ISUSER, DemoContext.getInstance().userMap.get(message.getTargetId()) == null);

        in.putExtra(MainActivity.NotifyRefreshReceiver.TARGETID, message.getTargetId());
        mContext.sendBroadcast(in);

    }

    public static final String UNREAD_COUNT_KEY = "UNREAD_COUNT_KEY";
...
//        return new RongIMClient.UserInfo(userId, "张三", "file:///storage/sdcard0/share.jpg");//测试本地图片
        ...
        Log.e("getUserInfo=====", "=====" + DemoContext.getInstance().getUserInfoById(userId));
        RongIMClient.UserInfo info = DemoContext.getInstance().getUserInfoById(userId);
        if (info == null) {
            info = new RongIMClient.UserInfo(userId, "未知用户", "");
        }
        return info;
    }
    ...
    public RongIMClient.Group getGroupInfo(String groupId) {
        ...
        RongIMClient.Group g = DemoContext.getInstance().getGroupMap().get(groupId);
        Log.i("getGroupInfo=====", "====="+g);
        if (g == null) {
            g = new RongIMClient.Group(groupId, "未知群", null);
            if (RCloudContext.getInstance() != null) {
                RCloudContext.getInstance().getGroupCache().put(g.getId(), g);
            }
        }
        return g;
    }
    ...
    public void onChanged(ConnectionStatus status) {
        ...
        switch (status) {
            case KICKED_OFFLINE_BY_OTHER_CLIENT:
                LocalBroadcastManager.getInstance(this.mContext.getApplicationContext())
